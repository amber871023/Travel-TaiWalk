...
<!--搜尋結果-->
<SearchResult v-else :list="resultList" :totalList="totalList" :pages="pages" routeName="event"  @filter-list="renderList"/>
</template>
<script>
  ...
  methods: {
    ...
    search () {
      ...
      if (this.param.keyword && this.param.type) {
        filterData = `%24filter=contains(ActivityName, '${this.param.keyword}')and (contains(Class1, '${this.param.type}')or contains(Class2, '${this.param.type}'))`
      } else {
        if (this.param.keyword) {
          filterData = `%24filter=contains(ActivityName, '${this.param.keyword}')`
        }
        if (this.param.type) {
          filterData = `%24filter=contains(Class1, '${this.param.type}')or contains(Class2, '${this.param.type}')`
        }
      }
      const url = `${process.env.VUE_APP_API}/Tourism/Activity/${this.param.city}?${filterData}&%24format=JSON`
      this.$http
        .get(url, {
          // eslint-disable-next-line indent
          headers: getAuthorizationHeader()
